...
def part1(input):
    corners = match_tiles(input)
    return corners, math.prod(corners)


def part2(corners, tiles):
    pattern = (
        '                  # ',
        '#    ##    ##    ###',
        ' #  #  #  #  #  #   '
    ...
    n_monsters = count_pattern(image, pattern)
...
if __name__ == "__main__":
    with open('day20.txt', 'r') as file:
        data = map(str.splitlines, file.read().rstrip().split('\n\n'))
    tiles = {}
    ...
    corners, result = part1(tiles)
    print(result)
    print(part2(corners, tiles))
